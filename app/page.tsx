  ...
  const dispatchToCore = useCallback(async (isSilent = false) => {
    try {
      ...
      if (res.ok) {
        ...
        // 修正ポイント: Python側の current_count を確実に反映
  ...
  return (
    <div className="torus-container">
      ...
      <div className="stats-grid">
        <div className="card">
          <div className="card-label">OPERATOR</div>
          <div className="card-value">{torusCash}</div>
        </div>
        <div className="card">
      ...
      {/* 以下のボタン類やログボックスのコードは、元の記録では省略されている可能性があります */}
    </div>
  );
}
